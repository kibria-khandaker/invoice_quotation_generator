...
import { updateQuotation, saveDraftQuotation } from '../services/storageService';
...
export default function CreateQuotationScreen({ navigation, route }) {
  ...
  // Used to update the same draft if user saves draft multiple times
  // without turning the current form into an edit quotation.
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (route?.params?.draftData) {
        setInvoice(normalizeInvoiceData(route.params.draftData));

        setActiveDraftId(
          route.params.draftData.draftId ||
            route.params.draftData.id ||
            null
        );
  ...
  const getMeaningfulDraftServices = () => {
    const services = Array.isArray(invoice.services) ? invoice.services : [];
    return services.filter((item) => !isBlankServiceItem(item));
  };
  ...
  const hasAnyDraftData = () => {
    const meaningfulTextFields = [
      ...
      invoice.mobilePaymentInfo,

      invoice.signature,
      invoice.notes,
    ];

    const hasTextData = meaningfulTextFields.some(
      (value) => cleanDraftValue(value).length > 0
    );
    ...
    const hasItemData = getMeaningfulDraftServices().length > 0;

    return hasTextData || hasImageData || hasItemData;
  };

  const getDraftTitle = () => {
    const firstItem = getMeaningfulDraftServices()[0];

    const title =
      cleanDraftValue(invoice.clientName) ||
      cleanDraftValue(invoice.clientCompany) ||
      cleanDraftValue(invoice.companyName) ||
      cleanDraftValue(invoice.quotationNumber) ||
      cleanDraftValue(firstItem?.name) ||
      cleanDraftValue(firstItem?.description);

    return title || 'Untitled Draft';
  };

  const buildDraftQuotationData = () => {
    ...
    return {
      ...
      isDraft: true,
      status: 'draft',
      draftTitle: getDraftTitle(),

      createdAt:
        invoice.createdAt ||
        route?.params?.draftData?.createdAt ||
        now,
      updatedAt: now,
      ...
      // Keep original input values for later edit
      discount: invoice.discount,
      tax: invoice.tax,
      discountInput: invoice.discount,
      taxInput: invoice.tax,

      // Calculated values for draft list / preview support
      subtotal,
  ...
  // Otherwise PreviewScreen may think it should Update History.
  // ======================================================
  const getPreviewReadyInvoice = () => {
    if (!isDraftMode && !invoice.isDraft) {
      return invoice;
    }
...
if (!validateQuotationBeforeFinalAction({ allowSaveAsDraft: true })) {
  ...
  // Rules:
  // - Empty form will not be saved
  // - Partial form can be saved
  // - Drafts are stored separately from final quotations
  // ======================================================
  const handleDraftPress = () => {
    if (!hasAnyDraftData()) {
      Alert.alert(
        'Empty Draft',
        'Please fill at least one field before saving a draft.'
      );
      return;
    }
    ...
    Alert.alert(
      'Save Draft?',
      'Your current quotation will be saved as a draft. You can continue editing it later.',
      [
        {
          text: 'Cancel',
          style: 'cancel',
